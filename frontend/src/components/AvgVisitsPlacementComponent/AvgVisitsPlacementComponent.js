...
function AvgVisitsPlacementComponent(props) {
	...
	function checkValidDateTime() {
		...
		let stringStartTime = getDateTime(startDate.current.value, "00:00");
		let stringEndTime = getDateTime(endDate.current.value, "00:00");
	...
	// -----------------------------
	async function getAvgVisitsPlacementByTimePeriod() {
		let TimePeriod = {
			StartDateTime: getDateTime(startDate.current.value, "00:00"),
			EndDateTime: getDateTime(endDate.current.value, "00:00")
		};
		/*
			axios requests :
		...
		*/
		try {
			const response = await axios.post(
				"/Placements/GetAvgVisitsPlacementByTimePeriod/placementId:" + props.placementId,
				TimePeriod,
				config
			);
			if (response.status === 200) {
				if (response.data.value != null) {
					setVisits(response.data.value.numberOfVisitsOfPlacementByTimePeriod.count / response.data.value.days);
				}
			}
		} catch (err) {
	...
	const submit = async e => {
		if (checkValidDateTime()) {
			await getAvgVisitsPlacementByTimePeriod();
		}
	};
	return (
		<div className="container m-2">
			...
			{visits != null ? (
				<div className="d-inline-flex justify-content-start w-100 mb-5">
					<div className="w-100">
						{/* // TODO language */}
						<h5>AVG number of people visited placement by this time period: {visits}</h5>
					</div>
				</div>
			) : (
				<div></div>
...
export default AvgVisitsPlacementComponent;
